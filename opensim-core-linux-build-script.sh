...
Help() {
    ...
    echo "    -p         specify the root path for build workspace and installation ~/opensim"
    echo
    exit
}

# Get flag values if any.
while getopts 'j:d:s:c:n:p:' flag
do
    case "${flag}" in
        j) NUM_JOBS=${OPTARG};;
        d) DEBUG_TYPE=${OPTARG};;
...
mkdir -p $OSPATH/workspace || true

# Create install folder
mkdir -p $OSPATH/install || true
...
if [[ $cmake_major_less == 1 ]] || $( [[ $cmake_major_equal == 1 ]] && [[ $cmake_minor_less == 1 ]] ); then
    mkdir $OSPATH/workspace/cmake-3.23.3-source || true
    cd $OSPATH/workspace/cmake-3.23.3-source
    ...
    cd $OSPATH/workspace/cmake-3.23.3-source/cmake-3.23.3
    ./bootstrap
...
if [[ $python_major_less == 1 ]] || $( [[ $python_major_equal == 1 ]] && [[ $python_minor_less == 1 ]] ); then
    mkdir $OSPATH/workspace/Python-3.6.9-source || true
    cd $OSPATH/workspace/Python-3.6.9-source
    wget -nc -q --show-progress https://www.python.org/ftp/python/3.6.9/Python-3.6.9.tgz
    tar -xvzf Python-3.6.9.tgz
    cd $OSPATH/workspace/Python-3.6.9-source/Python-3.6.9
    ./configure
    make -j$NUM_JOBS
...
mkdir -p $OSPATH/workspace/swig-source || true && cd $OSPATH/workspace/swig-source
wget -nc -q --show-progress https://github.com/swig/swig/archive/refs/tags/v4.1.1.tar.gz
tar xzf v4.1.1.tar.gz && cd swig-4.1.1
sh autogen.sh && ./configure --prefix=$OSPATH/swig --disable-ccache
make && make -j$NUM_JOBS install  
echo

# Get opensim-core.
echo "LOG: CLONING OPENSIM-CORE..."
git -C $OSPATH/workspace/opensim-core-source pull || git clone https://github.com/opensim-org/opensim-core.git $OSPATH/workspace/opensim-core-source
cd $OSPATH/workspace/opensim-core-source
git checkout $CORE_BRANCH
echo

# Build opensim-core dependencies.
echo "LOG: BUILDING OPENSIM-CORE DEPENDENCIES..."
mkdir -p $OSPATH/workspace/opensim-core-dependencies-build || true
cd $OSPATH/workspace/opensim-core-dependencies-build
cmake $OSPATH/workspace/opensim-core-source/dependencies -DCMAKE_INSTALL_PREFIX=$OSPATH/install/opensim-core-dependencies -DSUPERBUILD_ezc3d=on -DOPENSIM_WITH_CASADI=$MOCO -DOPENSIM_WITH_TROPTER=$MOCO
cmake . -LAH
...
mkdir -p $OSPATH/workspace/opensim-core-build || true
cd $OSPATH/workspace/opensim-core-build
cmake $OSPATH/workspace/opensim-core-source -G"$GENERATOR" -DOPENSIM_DEPENDENCIES_DIR=$OSPATH/install/opensim-core-dependencies -DBUILD_JAVA_WRAPPING=on -DBUILD_PYTHON_WRAPPING=on -DOPENSIM_C3D_PARSER=ezc3d -DBUILD_TESTING=off -DCMAKE_INSTALL_PREFIX=$OSPATH/install -DOPENSIM_INSTALL_UNIX_FHS=off -DSWIG_DIR=$OSPATH/swig/share/swig -DSWIG_EXECUTABLE=$OSPATH/swig/bin/swig
cmake . -LAH
cmake --build . --config $DEBUG_TYPE -j$NUM_JOBS
echo
...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$OSPATH/install/opensim-core-dependencies/simbody/lib
ctest --parallel $NUM_JOBS --output-on-failure

# Install opensim-core.
echo "LOG: INSTALL OPENSIM-CORE..."
cd $OSPATH/workspace/opensim-core-build
cmake --install . --prefix $OSPATH/install
echo 'export PATH=$OSPATH/install/opensim-core/bin:$PATH' >> ~/.bashrc
source ~/.bashrc
